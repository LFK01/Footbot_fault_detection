...
def modify_dispersion_cpp_file(par_x_length: int,
                               par_y_length: int,
                               bot_number: int):
    filename = '/Users/lucianofranchin/Documents/Github_repos/argos3-examples' \
               '/controllers/footbot_homing/footbot_dispersion.cpp'
    file = open(filename, 'r')
    new_file_text = ''
    for file_line in file.readlines():
        if 'myfile.open("log_files_directory/' in file_line:
            if 'nominal' not in file_line:
                file_line = '        myfile.open("log_files_directory/dispersion_size{}x{}_bot_number{}_fault_" +\n' \
                    .format(par_x_length, par_y_length, bot_number)
            elif 'nominal' in file_line:
                file_line = '        myfile.open("log_files_directory/dispersion_size{}x{}_bot_number{}_nominal_" +\n' \
                    .format(par_x_length, par_y_length, bot_number)
    ...
    output_file = open('/Users/lucianofranchin/Documents/Github_repos/argos3-examples'
                       '/controllers/footbot_homing/footbot_dispersion.cpp', 'w')
...
def compute_parameters_and_edit_files(par_x_length: int,
    ...
    modify_dispersion_cpp_file(par_x_length=par_x_length,
